...
- **`Lifecycle.Reactive<T>()`** &ndash; A per-value reactive trigger. Declare it as a static, then bind an external value at the `useAction` site by calling it (`actions.useAction(Actions.User(user), handler)`). The handler fires whenever the bound value changes by `Object.is`, receiving the latest value as its payload. Unlike `Lifecycle.Update()`, it **does** fire on mount when the value is already defined &mdash; so values present on the first render (a hydrated React Query cache) still reach the handler. See the [reactive values recipe](./reactive-values.md) for the full firing table and the React Query bridge.
