...
Dispatching an omnicast action performs both legs in one call: the action fires locally through the normal dispatch pipeline (including value caching for late subscribers), and the `{ name, payload, channel? }` envelope is published to the server for every **other** client in the audience &mdash; the sender is excluded server-side, so nothing double-fires. Receiving Boundaries validate the payload against the action's schema before dispatching it locally; an envelope that fails validation raises `Reason.Rejected` through `Lifecycle.Fault` instead of reaching subscribers. Outgoing payloads are not runtime-parsed &mdash; the compiler already guarantees them, since the payload type is inferred from the same schema. Channeled omnicast actions work like channeled broadcasts &mdash; the channel rides the envelope and subscriber matching happens on each client. Without an `sse` config, omnicast actions degrade gracefully to plain broadcasts.

Connections hold a mutable set of tags, seeded from `sse.tags` and mutated from any handler via `context.actions.tag.add/remove/clear` (plus `tag.has` for ALL-of checks) &mdash; changing which `Audience.Private(...)` dispatches the client receives, with mutations re-applied automatically after reconnects. Private dispatches embed their required tags in the envelope, and receivers re-validate them against their **current** tag set on arrival &mdash; so an event that matched when sent, but races a tag removal while in flight, is quietly rejected on the frontend before any subscriber sees it.
